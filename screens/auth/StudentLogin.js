...
export default function StudentLogin({ navigation }) {
  ...
  const [showAlerts, setShowAlerts] = useState(true); // Control alerts

  const showAlert = (title, message) => {
    if (!showAlerts) return; // Skip alert if false
    if (Platform.OS === "web") {
      window.alert(`${title}\n\n${message}`);
    } else {
      alert(`${title}\n\n${message}`);
  ...
  useEffect(() => {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (user) => {
      setShowAlerts(false); // Disable alerts during auto-login
      if (user) {
        try {
          await user.reload();
          ...
          unsubscribeSnapshot = onSnapshot(studentDocRef, (docSnapshot) => {
            if (!docSnapshot.exists()) {
              signOut(auth);
              // No alert on auto deletion
            }
          });

          const userDoc = await getDoc(studentDocRef);
          if (userDoc.exists()) {
            navigation.replace("StudentDashboard", { studentId: user.uid });
          } else {
            await signOut(auth);
          }
        } catch (error) {
      ...
      setLoading(false);
      setShowAlerts(true); // Re-enable alerts for manual login
    });
  ...
  const handleLogin = async () => {
    setShowAlerts(true); // Enable alerts for manual login
    let hasError = false;
    ...
    try {
      ...
      if (!userDoc.exists()) {
        ...
        showAlert(
          "Account Missing",
          "This account has no student record. It may have been deleted."
        );
        return;
      }

      // Real-time deletion listener (manual login)
      onSnapshot(studentDocRef, (docSnapshot) => {
        if (!docSnapshot.exists()) {
          signOut(auth);
          showAlert(
            "Account Deleted",
            "Your student record has been deleted. You have been logged out."
          );
        }
      });
  ...
  const handleForgotPassword = async () => {
    setShowAlerts(true); // Enable alerts
    if (!email.trim()) {
      setEmailError(true);
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      <View style={styles.container}>
        ...
        <TextInput
          placeholder={emailError ? "Please fill in email" : "Enter email"}
        ...
        <View style={[styles.passwordContainer, passwordError && { borderColor: "red" }]}>
          <TextInput
            placeholder={passwordError ? "Please fill in password" : "Enter password"}
...
const styles = StyleSheet.create({
  ...
  button: { backgroundColor: "#FFA726", padding: 15, borderRadius: 25, alignItems: "center", marginTop: 20, width: "100%" },
